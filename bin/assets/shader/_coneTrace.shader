...
// _raycast
 ...
 vec4 raycastBrickQuadrilinear(in uint childColorU, in uint parentColorU, in vec3 dir, in vec3 childStart, in vec3 childEnd, in vec3 parentStart, in vec3 parentEnd, in uint childLevel, in float coneDiameter, in float travelDist, out float tLeave) {
    ...
    for (float f = 0; f < stepLength; f += stepSize) {
      ...
      if (newCol.a > 0.001) {  
        ...
        float falloff = max(1.0, (25 * currTravelDist) * (25 * currTravelDist));
        newCol /= falloff;
...
vec4 raycastBrick(in uint nodeColorU, in vec3 enter, in vec3 leave, in vec3 dir, in uint level, in float nodeSideLength, in float travelDist, out float tLeave) {
    ivec3 brickRes = textureSize(brickPool_color, 0); // TODO: make uniform
    float voxelStep = 1.0 / float(brickRes.x);
    float stepSize = voxelStep / 3; 
    vec4 color = vec4(0);
  ...
  //  color = texture(brickPool_color, brickAddressUVW + vec3(voxelStep));
    ...
    for (float f = 0; f < stepLength; f += stepSize) {
      ...
      vec4 newCol;
      float currTravelDist = travelDist + f * (nodeSideLength / 6);
      tLeave = currTravelDist;
...
vec4 coneTrace(in vec3 rayOriginTex, in vec3 rayDirTex, in float coneDiameter, in float maxDistance) {
  ...
  for (float f = tEnter + e; f < end; f += tLeave + e) {
    ...
    if (childAddress != int(NODE_NOT_FOUND)) {
       ...
       vec4 parentCol = raycastBrick(parentColorU, parentStart, parentEnd, rayDirTex, childLevel - 1, parentSize, f, /*out*/ tLeave);     
       vec4 childCol = raycastBrick(childColorU, childStart, childEnd, rayDirTex, childLevel, childSize, f, /*out*/ tLeave);
