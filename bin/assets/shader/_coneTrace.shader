...
vec4 coneTrace(in vec3 rayOriginTex, in vec3 rayDirTex, in float coneDiameter, in float maxDistance) {
  ...
  for (float f = tEnter + 0.00001; f < end; f += tLeave + 0.00001) {
    ...
    float targetSize = coneDiameter * 500.0 * f;
    
    float sampleDiameter = max(1.0 / float(LEAF_NODE_RESOLUTION), targetSize);
    float sampleLOD = abs(log2(1.0 / sampleDiameter));
    
    int addressFloor = traverseOctree_level(posTex, uint(floor(sampleLOD)),
                       nodePosMin, nodePosMax);

    sampleDiameter = 1.0 / float(pow2[targetLevel]);
    ...
    if (addressFloor != int(NODE_NOT_FOUND)) {
       ...
       vec3 enterPos = (posTex - nodePosMin) / sampleDiameter;
       
       vec3 leavePos = 
           ((posTex + rayDirTex * tLeave) - nodePosMin) / sampleDiameter;
       
       newCol = raycastBrick(nodeColorUf, enterPos, leavePos, rayDirTex,
                             targetLevel, sampleDiameter);
