...
#include "VoxelConeTracing/ObFlagPass.h"
...
ObFlagPass::ObFlagPass(VCTscene* vctScene) {
  ...
  _flagShader.init();
  _flagShader.setName("octreeVis shader");
 ...
 //         new FunctionOp(std::bind(&ObFlagPass::debugVoxelFragmentList, this)));

  nodePass->addOperation(new RenderMesh(fsqMeshComponent));
}


void ObFlagPass::debugVoxelIndexAC() {
...
void ObFlagPass::debugVoxelFragmentList() {
  uint byteSize =
