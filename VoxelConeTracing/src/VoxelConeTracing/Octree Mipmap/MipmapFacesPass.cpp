...
MipmapFacesPass::~MipmapFacesPass(void) {
}
...
MipmapFacesPass::
  ...
  _name = std::string("MipmapFaces (level: ").append(std::to_string(level).append(")"));
  ...
  if (mipmapMode == THREAD_MODE_COMPLETE) {
    shp->loadShader("./assets/shader/MipmapFaces.shader",
